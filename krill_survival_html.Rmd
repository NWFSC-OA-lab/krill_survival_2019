...
#moats 3 is a bit of an outlier. if it is removed, the fraility disappears
d <- dfAcclGoodMoats %>%
#  filter(Moats != 3) %>%
  {.}
...
# Note: the for loop can be kinda slow beacuse it is fitting all four models
...
#new data of treatmenst for survival plots
dNew <- data.frame(treatment = unique(dfAcclGoodMoats$treatment))
#predictions (surival probabilites) of treatmeents 
pred <- predict(fme, newdata = dNew, conf_int = "adjusted")

#create a data frame of the prections for ploting all the treatment curves
dPlot <- NULL
...
# TODO could and condifence bands or ribbons with dPlot$survival_m_l_a and dPlot$survival_m_r_a, but that would probably the graph too messy
...
```
Survival in the three treatments was compared using a hazard model that included the MOATS as a clustered frailty term. The frailty model was fit using the frailtyEM R package [ref]. Preliminary analyses was conducted to compare four potnetial distribution of the frailty (gamma, postive stable, inverse Gausian and non-central gamma). The postive stable distribution had the greated likelihood value and it was selected for the final analysis (Balan and Putter). Using a likelihood ratio test, inclusion of the frailty term was deemed significant when compared to the treatement only hazards model (p = 0.000282). Likewise, the presense of unexplained heterogenety in the simple non-frailty model suggests appropriate inclusion of the frailty (MOATS) term (Commenges-Andersen test p = 0.00417). There were no significant differences among pair-wise comparisons of the treatments (all vs. current p = 0.26; all vs. highT p = 0.38; current vs. highT p = 0.87). The graph plots the marginal survival curves comparing treatments integrated over the MOATS frailty effect.